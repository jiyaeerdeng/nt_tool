...
def convert_aa_quota(origin_quota: int) -> int:
    ...
    """
    return 9 if origin_quota == 0 else origin_quota
...
def calculate_aa_mix_by_segment(target_cabin_class: CabinClass, duration_list: List[timedelta],
    ...
    for x in cabin_exist_list:
        x.sort(reverse=True)
        if cabin_equal_or_lower_list := [
            v for v in x if target_cabin_class >= v
        ]:
            actual_cabin_list.append(cabin_equal_or_lower_list[0])
        elif len(x) == 1 and x[0] == CabinClass.F:
    ...
    if all(x == target_cabin_class for x in actual_cabin_list):
        ...
        is_mix = True
        mix_detail = '+'.join(
            f'{str(round(percentage_list[x] * 100))}%' + actual_cabin_list[x]
...
def convert_ac_response_to_models(response: requests.Response) -> List:
    ...
    for r in air_bounds_json:
        r = dict(r)
        prices_raw = list(r['airBounds'])
        segs_raw = list(r['boundDetails']['segments'])
        prices = []
        for pr in prices_raw:
            ...
            if pr['fareFamilyCode'] in cabin_class_dict and all(
                'AC' not in str(x['flightId']).split('-')[1]
                or (
                    x['bookingClass'] in saver_class_list
                    and 'AC' in str(x['flightId']).split('-')[1]
                )
                ...
                temp_pricing = Pricing(
                    ...
                    miles=pr['airOffer']['milesConversion']['convertedMiles'][
                        'base'
                    ],
                    excl_cash_in_cents=pr['airOffer']['milesConversion'][
                        'convertedMiles'
                    ]['totalTaxes'],
                    excl_currency='CAD',
                    is_mix=pr.get('isMixedCabin', False),
                    mix_detail=convert_mix(pr['availabilityDetails'])
                    if pr.get('isMixedCabin', False)
                    else "N/A",
                )
...
def convert_aa_response_to_models(response: requests.Response) -> List:
    ...
    for r in air_bounds_json:
        r = dict(r)
        segs_raw = list(r['segments'])
        segs = []
        for sg in segs_raw:
            temp_seg = Segment(
                flight_code=sg['flight']['carrierCode']
                + sg['flight']['flightNumber'],
                aircraft=sg['legs'][0]['aircraft']['code'],
                departure=sg['origin']['code'],
                excl_departure_time=sg['departureDateTime'],
                excl_cabin_exist=list(
                    {
                        cabin_class_dict[x['cabinType']]
                        for x in sg['legs'][0]['productDetails']
                    }
                ),
                arrival=sg['destination']['code'],
                excl_arrival_time=sg['arrivalDateTime'],
                excl_duration_in_seconds=sum(
                    x.get('durationInMinutes', 0) * 60 for x in sg['legs']
                ),
                excl_connection_time_in_seconds=sum(
                    x.get('connectionTimeInMinutes', 0) * 60
                    for x in sg['legs']
                ),
            )
            segs.append(temp_seg)
